...
interface SolarQuoteCalculatorProps {
  gridData: GridTieSystemData[]; largeData: LargeSystemData[]; loading: boolean;
}
...
const SolarQuoteCalculator = ({ gridData, largeData, loading }: SolarQuoteCalculatorProps) => {
  ...
  const handleCalculate = () => {
    ...
    if (kw <= RESIDENTIAL_THRESHOLD_KW) {
      ...
      const closestSystem = gridData.reduce((prev, curr) => 
        Math.abs(curr.systemSize - kw) < Math.abs(prev.systemSize - kw) ? curr : prev
      );
      pricePerWatt = closestSystem.hdgElevatedWithGst;
      inverterSizeKw = closestSystem.inverterCapacity;
      estimateBasis = `Based on ${closestSystem.systemSize} kWp Residential System`;
    } else {
      if (!largeData || largeData.length === 0) {
        setError("Commercial pricing data is not available. Cannot calculate.");
        setResult(null); return;
      }
      const closestSystem = largeData.reduce((prev, curr) => 
        Math.abs(curr.systemSizeKWp - kw) < Math.abs(prev.systemSizeKWp - kw) ? curr : prev
      );

      switch (mountingType) {
        case 'tin-shed':
          pricePerWatt = closestSystem.shortRailTinShedPricePerWatt;
          estimateBasis = `Based on Commercial (Tin Shed) pricing`;
          break;
        case 'pre-gi-mms':
          pricePerWatt = closestSystem.preGiMmsPricePerWatt;
          estimateBasis = `Based on Commercial (Pre GI MMS) pricing`;
          break;
        case 'without-mms':
          pricePerWatt = closestSystem.priceWithoutMmsPricePerWatt;
          estimateBasis = `Based on Commercial (Without MMS) pricing`;
          break;
        case 'rcc-elevated':
        default:
          pricePerWatt = closestSystem.hdgElevatedRccPricePerWatt;
          estimateBasis = `Based on Commercial (RCC Elevated) pricing`;
          break;
      }
      inverterSizeKw = closestSystem.inverterCapacity;
    }
    ...
    setResult({
      userInputKw: kw, numberOfPanels, actualSystemSizeKwp, inverterSizeKw, pricePerWatt, basePrice, gstAmount, totalPrice, estimateBasis,
    });
  };
...
export default function Reliance() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [gridRes, largeRes, cablesRes, kitsRes, configRes] = await Promise.all([
          supabase.from('reliance_grid_tie_systems').select('*').order('sl_no', { ascending: true }), supabase.from('reliance_large_systems').select('*').order('sl_no', { ascending: true }), supabase.from('reliance_dc_cable_data').select('*').order('sr_no', { ascending: true }), supabase.from('reliance_kit_items').select('*').order('sr_no', { ascending: true }), supabase.from('reliance_system_config').select('*'),
        ]);
        if (gridRes.data) {setGridData(gridRes.data.map((r: any) => ({slNo: r.sl_no, systemSize: Number(r.system_size), noOfModules: r.no_of_modules, inverterCapacity: Number(r.inverter_capacity), phase: r.phase, hdgElevatedWithGst: Number(r.price_per_watt ?? r.hdg_elevated_with_gst ?? 0), hdgElevatedPrice: Number(r.hdg_elevated_price ?? 0),})))}
        if (largeRes.data) {setLargeData(largeRes.data.map((r: any) => ({slNo: r.sl_no, systemSizeKWp: Number(r.system_size_kwp), systemSizeKW: Number(r.system_size_kw), noOfModules: r.no_of_modules, inverterCapacity: Number(r.inverter_capacity), phase: r.phase, shortRailTinShedPricePerWatt: Number(r.short_rail_tin_shed_price_per_watt), shortRailTinShedPrice: Number(r.short_rail_tin_shed_price), hdgElevatedRccPricePerWatt: Number(r.hdg_elevated_rcc_price_per_watt), hdgElevatedRccPrice: Number(r.hdg_elevated_rcc_price), preGiMmsPricePerWatt: Number(r.pre_gi_mms_price_per_watt), preGiMmsPrice: Number(r.pre_gi_mms_price), priceWithoutMmsPricePerWatt: Number(r.price_without_mms_price_per_watt), priceWithoutMmsPrice: Number(r.price_without_mms_price),})))}
        if (cablesRes.data) {setCableData(cablesRes.data.map((r: any) => ({srNo: r.sr_no, productDescription: r.product_description, uom: r.uom, quantity: r.quantity, price: Number(r.price), total: Number(r.total),})))}
        if (kitsRes.data) {setKitData(kitsRes.data.map((r: any) => ({srNo: r.sr_no, item: r.item, description: r.description,})))}
        if (configRes.data) { const config = Object.fromEntries(configRes.data.map((c: any) => [c.key, c.value])); if (config['PRODUCT_DESCRIPTION']) setProductDescription(config['PRODUCT_DESCRIPTION']); if (config['COMMERCIAL_SYSTEM_SIZE_LIMIT']) setCommercialLimit(parseFloat(config['COMMERCIAL_SYSTEM_SIZE_LIMIT']));}
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <SolarQuoteCalculator gridData={gridData} largeData={largeData} loading={loading} />
        <Tabs defaultValue="residential" className="w-full">
