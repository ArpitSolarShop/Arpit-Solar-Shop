...
// function RelianceCommercialTable({ data, onRowClick }: { data: any[]; onRowClick: (product: any) => void }) {
...
//                 <TableCell className="font-bold text-green-600">₹{item.shortRailTinShedPrice.toLocaleString("en-IN")}</TableCell>
//                 <TableCell className="font-medium">₹{item.hdgElevatedRccPricePerWatt.toFixed(2)}</TableCell>
//                 <TableCell className="font-bold text-green-600">₹{item.hdgElevatedRccPrice.toLocaleString("en-IN")}</TableCell>
//                 <TableCell className="font-medium">₹{item.preGiMmsPricePerWatt.toFixed(2)}</TableCell>
//                 <TableCell className="font-bold text-green-600">₹{item.preGiMmsPrice.toLocaleString("en-IN")}</TableCell>
//                 <TableCell className="font-medium">₹{item.priceWithoutMmsPricePerWatt.toFixed(2)}</TableCell>
//                 <TableCell className="font-bold text-green-600">₹{item.priceWithoutMmsPrice.toLocaleString("en-IN")}</TableCell>
//                 <TableCell className="font-medium text-green-600">
...
// function PriceComparison({ data }: { data: any[] }) {
...
//         // Do not auto-fill cables from this page
//       />
//         <Footer />
//     </div>
//   )
...
// // Components for pricing tables
// function RelianceCommercialTable({ data, onRowClick }: { data: SolarSystem[]; onRowClick: (product: SolarSystem) => void }) {
...
// const customQuoteTemplate: SolarSystem = {
//   slNo: 0,
//   systemSize: 0,
//   noOfModules: 0,
//   inverterCapacity: 0,
...
//       const { data } = await supabase.from('reliance_large_systems').select('*').order('sl_no', { ascending: true })
//       if (data) {
//         setRows(data.map((r: any): SolarSystem => {
//           const systemSize = Number(r.system_size_kwp)
//           const d = computeDerived(systemSize, Number(r.short_rail_tin_shed_price))
//           return {
//             slNo: r.sl_no,
...
//   const handleRelianceRowClick = (product: SolarSystem) => {
//     setSelectedRelianceProduct(product)
//     setIsRelianceFormOpen(true)
//   }
...
//                     className="border-gray-400 text-gray-700 hover:bg-gray-200 bg-transparent"
//                     onClick={() => {
//                       setSelectedRelianceProduct(customQuoteTemplate)
//                       setIsRelianceFormOpen(true)
//                     }}
//                   >
//                     Get Reliance Solar Quote
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50">
              ...
              <TableHead className="font-semibold">Monthly Generation (kWh)</TableHead>
              <TableHead className="font-semibold">Roof Area (sq ft)</TableHead>
              <TableHead className="font-semibold text-center">Tin Shed (₹)</TableHead>
              <TableHead className="font-semibold text-center">RCC Elevated (₹)</TableHead>
              <TableHead className="font-semibold text-center">Pre GI MMS (₹)</TableHead>
              <TableHead className="font-semibold text-center">Without MMS (₹)</TableHead>
              <TableHead className="font-semibold">Monthly Savings (₹)</TableHead>
              <TableHead className="font-semibold">Payback (Years)</TableHead>
              <TableHead className="font-semibold">CO₂ Reduction (tons/year)</TableHead>
            </TableRow>
