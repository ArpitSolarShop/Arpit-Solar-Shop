...
// // Updated Deno serverless function with DoubleTick template sending
// import { serve } from "https://deno.land/std@0.190.0/http/server.ts";
// import { createClient } from "https://esm.sh/@supabase/supabase-js@2.52.0";
...
// // ---- Your DoubleTick details ----
// const DOUBLE_TICK_API_KEY =
//   "key_o6Dp7MBLIwKlpKHlqcf4VaI8eGEyGkWfp76gluNY0gjjd3T5EuUblUNsbgqGMzj7LZhDfwbuoLbDxU8LTGehJW1m0sSDhqDvf2GAw4puBEAfInI5qV13rWPjpNPrvw812bitePsseEFcnJavcAOlVfqqg0bJoOA15DI06zDAhOhbXai7xW7LFWt0DdDpuby7kWHGc3pcsRrCqUGPDRvnjfSfBtlMcxwXzLJyi27Y6Mh4hfjcyU1bu1eZBmGo";
// const SENDER_NUMBER = "+919044555572"; // Registered sender number
...
//     // Send WhatsApp template with PDF link
...
// function generateRelianceHTML(formData: QuoteFormData) {
//   return `<!DOCTYPE html><html><body><h1 style="color:#2563eb">Reliance HJT Quotation</h1><p>Name: ${formData.name}</p><p>Phone: ${formData.phone}</p><p>Product: ${formData.product_name}</p></body></html>`;
// }

// function generateShaktiHTML(formData: QuoteFormData) {
//   return `<!DOCTYPE html><html><body><h1 style="color:green">Shakti Solar Quote</h1><p>Name: ${formData.name}</p><p>Phone: ${formData.phone}</p><p>Power: ${formData.power_demand_kw || "N/A"} kW</p></body></html>`;
// }

// function generateDefaultHTML(formData: QuoteFormData) {
//   return `<!DOCTYPE html><html><body><h1>Solar Quote</h1><p>Name: ${formData.name}</p><p>Phone: ${formData.phone}</p></body></html>`;
// }

// function createSimplePDF(formData: QuoteFormData) {
...
//               placeholders: [
//                 formData.name,
//                 pdfUrl // Send PDF link as placeholder
//               ]
//             }
//           }
//         }
...
//       method: "POST",
//       headers: {
//         Authorization: DOUBLE_TICK_API_KEY,
//         "Content-Type": "application/json"
//       },
...
//   const result = await response.json();
//   if (!response.ok) throw new Error(result.message || "DoubleTick API error");
//   return result;
// }
...
// Updated Deno serverless function: Send PDF as WhatsApp Document (with fallback template)
import { serve } from "https://deno.land/std@0.190.0/http/server.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.52.0";
...
serve(async (req: Request) => {
  ...
  try {
    ...
    let whatsappResult;
    try {
      whatsappResult = await sendWhatsAppPDF(formData.phone, signedUrlData.signedUrl, formData);
    } catch (docError) {
      console.error("Document send failed, falling back to template:", docError);
      whatsappResult = await sendWhatsAppTemplate(formData.phone, signedUrlData.signedUrl, formData);
    }
...
// ---------- WhatsApp Sending ----------
async function sendWhatsAppPDF(phone: string, pdfUrl: string, formData: QuoteFormData) {
  const cleanPhone = phone.replace(/[^\d+]/g, "");

  const payload = {
    to: cleanPhone,
    document_url: pdfUrl,
    filename: `Quote_${formData.name.replace(/\s+/g, "_")}.pdf`,
    caption: `Hi ${formData.name}, here is your solar quotation for ${formData.product_name || "our products"}.`
  };

  const response = await fetch("https://public.doubletick.io/whatsapp/message/document", {
    method: "POST",
    headers: {
      Authorization: DOUBLE_TICK_API_KEY,
      "Content-Type": "application/json",
  ...
  const result = await response.json();
  if (!response.ok) throw new Error(result.message || "DoubleTick Document API error");
  return result;
}

async function sendWhatsAppTemplate(phone: string, pdfUrl: string, formData: QuoteFormData) {
  const cleanPhone = phone.replace(/[^\d+]/g, "");
    ...
    messages: [
      {
        ...
        content: {
          ...
          templateData: {
            body: {
              placeholders: [formData.name, pdfUrl]
            }
          }
        }
      }
  ...
  const result = await response.json();
  if (!response.ok) throw new Error(result.message || "DoubleTick Template API error");
  return result;
}
